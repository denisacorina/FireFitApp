...
namespace FoodObjectDetection
{
    public partial class ObjectDetection
    {

        public const string RetrainFilePath = @"C:\Users\DENI\source\repos\FireFitBlazor\FoodDetection\bin\Debug\net9.0\annotations.json";
        public const string OutputModelPath = @"C:\Users\DENI\source\repos\FireFitBlazor\FoodDetection\bin\Debug\net9.0\ObjectDetection.mlnet";
        ...
        public static void Train(string outputModelPath = OutputModelPath, string inputDataFilePath = RetrainFilePath)
        {
        ...
        private static IEnumerable<ModelInput> LoadFromCoco(string inputDataFilePath)
        {
            ...
            var allAnnotations = jsonNode["annotations"].AsArray();

            // Step 1: Count category frequency
            var categoryCounts = allAnnotations
                .GroupBy(a => a["category_id"].GetValue<int>())
                .ToDictionary(g => g.Key, g => g.Count());

            // Step 2: Select top 15 most common category IDs
            var topCategoryIds = categoryCounts
                .OrderByDescending(kv => kv.Value)
                .Skip(1).Take(15)
                .Select(kv => kv.Key)
                .ToHashSet();

            // Step 3: Filter categories dictionary
            ...
            // Step 5: Filter annotations to only top categories
            var annotations = allAnnotations.Select(node =>
            {
                var imageId = node["image_id"].GetValue<int>();
                var categoryId = node["category_id"].GetValue<int>();
                var bbox = node["bbox"].AsArray().Select(bbox => bbox.GetValue<float>()).ToArray();
            ...
            Console.WriteLine($"Filtered annotations count: {annotations.Count()}");
            Console.WriteLine($"Filtered annotations count: {annotations}\n");

            // Step 6: Group and prepare ModelInput
            // Limit to at most 15,000 images
            foreach (var group in annotations.GroupBy(a => a.ImageId).Take(10000))
            {
                if (!images.ContainsKey(group.Key)) continue;

                var image = images[group.Key];
                var sourceWidth = image.ImageWidth;
                var sourceHeight = image.ImageHeight;

                CalculateAspectAndOffset(sourceWidth, sourceHeight, TrainingImageWidth, TrainingImageHeight, out float xOffset, out float yOffset, out float aspect);

                var labels = group.Select(a => categories[a.CategoryId]).ToArray();
                var boxes = group.Select(a => new[]
                {
        xOffset + (a.Bbox[0] * aspect),
        yOffset + (a.Bbox[1] * aspect),
        xOffset + ((a.Bbox[0] + a.Bbox[2]) * aspect),
        yOffset + ((a.Bbox[1] + a.Bbox[3]) * aspect),
    }).SelectMany(x => x).ToArray();
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.Conversion.MapValueToKey(outputColumnName:@"Labels",inputColumnName:@"Labels",addKeyValueAnnotationsAsText:false)      
                                    ...
                                    .Append(mlContext.MulticlassClassification.Trainers.ObjectDetection(new ObjectDetectionTrainer.Options(){LabelColumnName=@"Labels",PredictedLabelColumnName=@"PredictedLabel",BoundingBoxColumnName=@"Box",ImageColumnName=@"Image",ScoreColumnName=@"score",MaxEpoch=3,InitLearningRate=1,WeightDecay=0,}))      
